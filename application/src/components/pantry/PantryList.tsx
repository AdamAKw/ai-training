...
export function PantryList({ items, onEdit, onDelete }: PantryListProps) {
   ...
   const itemsByCategory = items.reduce<Record<string, IPantryItem[]>>((acc, item) => {
   ...
   if (items.length === 0) {
      return (
         <div className="text-center py-8">
            <p className="text-muted-foreground">Twoja spiżarka jest pusta. Dodaj produkty, aby zacząć.</p>
         </div>
      );
   }
            ...
            <div key={category}>
               ...
               <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
                  {itemsByCategory[category].map((item) => (
                     <PantryItem key={item._id as string} item={item} onEdit={onEdit} onDelete={onDelete} />
                  ))}
               </div>
            </div>
         ))}
