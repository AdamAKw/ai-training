...
export function IngredientAvailability({ ingredients, pantryItems, servings }: IngredientAvailabilityProps) {
   const availabilityMap = checkIngredientsAvailability(ingredients, pantryItems, servings);

   // Count how many ingredients are available vs. missing
   const totalIngredients = ingredients.length;
   const availableIngredients = Array.from(availabilityMap.values()).filter((item) => item.available).length;
            ...
            {ingredients.map((ingredient, index) => {
               ...
               return (
                  <li key={index} className="flex items-center gap-2">
                     ...
                     <span>
                        {ingredient.name} ({(ingredient.quantity * servings).toFixed(1)} {ingredient.unit})
                        {status.available ? (
